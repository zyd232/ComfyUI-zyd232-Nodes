...
- **frame_step** — Frame sampling interval per reference video (default `1`). Keeps 1 frame out of every `frame_step` source frames (`n = ceil(total / frame_step)`), then capped by `max_video_frames`. Set to `1` to keep every frame.
...
| **frame_step** | Frame sampling interval per reference video; keeps 1 frame out of every `frame_step` source frames (`n = ceil(total / frame_step)`), capped by `max_video_frames` | Using video inputs |
